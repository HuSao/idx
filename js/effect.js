...
let $iInnovationI = $("#iInnovationI");
let $iInnovationT = $("#iInnovationT");

let $iExploreI = $("#iExploreI");
let $iExploreT = $("#iExploreT");

$iInnovationI.css("display", "none");
$iInnovationT.css("display", "none");

$iExploreI.css("display", "none");
$iExploreT.css("display", "none");

let fadeTime = 3000;

function toggle(parameter) {
    if (parameter === 1) {
        $iExploreI.fadeOut(fadeTime);
        $iExploreT.fadeOut(fadeTime);

        $iFocusI.fadeIn(fadeTime);
        $iFocusT.fadeIn(fadeTime);
    } else if (parameter === 2) {
        $iFocusI.fadeOut(fadeTime);
        $iFocusT.fadeOut(fadeTime);

        $iInnovationI.fadeIn(fadeTime);
        $iInnovationT.fadeIn(fadeTime);
    } else if (parameter === 3) {
        $iInnovationI.fadeOut(fadeTime);
        $iInnovationT.fadeOut(fadeTime);

        $iExploreI.fadeIn(fadeTime);
        $iExploreT.fadeIn(fadeTime);
    }
}


let $iAboutForm = $("#iAboutForm");

$iAboutForm.css("display", "none");

function aboutClick() {
    $iAboutForm.fadeIn(3000);
}

function aboutCloseClick() {
    $iAboutForm.fadeOut(3000);
}
...
const EffectConfetti = (() => {
    ...
    C.DefaultConfig = {
        ...
        size: 10, interval: 16, timeout: 3000, posJitter: 0, angJitter: 0, distJitter: 0, sizeJitter: 0
    };
    ...
    C.getAngDist = ([x0, y0], [x1, y1]) => {
        ...
        return [(Math.atan2(dy, dx) * 180) / Math.PI + 90, Math.round(Math.sqrt(dx * dx + dy * dy))];
    };

    P.fireConfetti = function (from, to) {
        const {
            parent, colors, anims, timeout, posJitter, angJitter, distJitter, size, sizeJitter
        } = this.config;
...
(async () => {
    ...
    let fxConfetti = new EffectConfetti({
        parent, size: 10, posJitter: 20, angJitter: 15, distJitter: 50, sizeJitter: 10
    });
